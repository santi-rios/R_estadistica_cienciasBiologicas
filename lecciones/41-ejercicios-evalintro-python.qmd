...
- Existen 3 variables que se refieren a `país` (`country`, `iso2`, `iso3`). Vamos a quitar 2 de estas 3 variables (`iso2`, `iso3`).
...
- Después, tenemos una variable que indica año (`year`). Explora qué tipo de variable es y conviértela a una variable de tipo `category`.

```{pyodide}
#| envir: myenv
#| exercise: examen1_3

# Convierte la columna 'year' a tipo 'category'
oms_3 = oms_2.assign(year = oms_2['year'].astype(______))
...
::: {.solution exercise="examen1_3"}
```python
oms_3 = oms_2.assign(year = oms_2['year'].astype('category'))
oms_3.info()
```
:::
...
::: {layout-ncol="2"}
![](https://i.imgur.com/52PlC5i.png)
![](https://i.imgur.com/fT3fTWS.png)
:::
...
- Al correr el siguiente código, deberías tener un dataframe con 4 columnas: `country`, `year`, `clave` y `casos`.
...
Las columnas que no se "derriten" son `country` y `year`.
:::

::: {.solution exercise="examen1_4"}
```python
oms_4 = oms_3.melt(
...
oms_limpio = (oms
    ...
    .melt(id_vars=['country', 'year'], var_name='clave', value_name='casos')
    .dropna(subset=['casos'])
    .assign(
